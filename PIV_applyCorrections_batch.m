...
saveUnrotateduser = true;   % ← set false for full production run
...
for loopNo = 1%:numel(loopFolders)
    ...
    for camNo = 1:numel(cameraList)
        ...
        if ~isfolder(instDir)
            ...
            % then in the camera loop:
            PIV_applyVelocityRotation(instDir, p_C_all{camNo}, SaveUnrotated=saveUnrotateduser);
        end
